...
// Generous window: the ESP polls every ~2s and each bind can take up to 15s,
// and binds can queue behind one another, so allow plenty of time.
...
export function LearnModal({
  ...
  // Keep latest props in refs so the realtime subscription effect can depend
  // only on [open, slot] and not churn on every parent re-render.
  ...
  // Realtime subscription: stays alive the ENTIRE time the modal is open,
  // independent of the countdown. This is the key fix — a late rf_bound (because
  // the ESP's bind command was queued/delayed) still marks the slot learned,
  // even after the visual countdown has elapsed.
  useEffect(() => {
    if (!open || slot === null) return;

    const channel = supabase
      .channel(`rf_learn_${slot}_${Date.now()}`)
      .on(
        "postgres_changes",
  ...
  // Countdown timer — purely visual. Flips to the "timeout" screen when it runs
  // out, but does NOT stop the subscription above, so a late capture still wins.
  useEffect(() => {
